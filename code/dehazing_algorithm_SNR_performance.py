...
def calculate_snr(signal, noise):
    '''
    Calculates the Signal-to-Noise Ratio (SNR) for a given signal and noise.
    The SNR is defined as the ratio of the power of a signal to the power of background noise.

    Parameters:
        signal (numpy.ndarray): The video frame considered as the signal.
        noise (numpy.ndarray): The noise in the video frame.
...
while True:
   ...
   # Calculate and store SNR for the original frame using a reference noise model
    original_noise = img - np.mean(img, axis=0)  # Assuming mean deviation as noise
    original_snr = calculate_snr(img, original_noise)
    original_snr_values.append(original_snr)

    # Calculate and store SNR for the enhanced frame
    enhanced_noise = J_contrast_enhanced - np.mean(J_contrast_enhanced, axis=0)
    enhanced_snr = calculate_snr(J_contrast_enhanced, enhanced_noise)
    enhanced_snr_values.append(enhanced_snr)
...
# Calculate and print average SNRs
average_original_snr = np.mean(original_snr_values)
average_enhanced_snr = np.mean(enhanced_snr_values)
print(f"Average SNR for the original video: {average_original_snr:.2f} dB")
print(f"Average SNR for the enhanced video: {average_enhanced_snr:.2f} dB")
print(f"Improvement in SNR: {average_enhanced_snr - average_original_snr:.2f} dB")
